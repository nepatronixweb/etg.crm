...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file.size > MAX_BYTES) {
      return NextResponse.json({ error: "QR image must be 5 MB or smaller" }, { status: 400 });
    }
    ...
    await deleteBrandingFileByPath(settings.paymentQrPath);
    const paymentQrPath = await uploadBrandingFile(buffer, file.name, file.type, {
